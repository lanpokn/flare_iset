...
for i = 1:loop
    ...
    for ii = 1:numel(nsides)
        ...
        [aperture, params] = wvfAperture(wvf,'nsides',nsides(ii),...
            'dot mean',100, 'dot sd',50, 'dot opacity',0.6,'dot radius',30+100*rand(),...
            'line mean',100, 'line sd',50, 'line opacity',0.6,'linewidth',10+50*rand(),'segmentlength',3000+4500*rand());
        ...
        oi = oiAdjustIlluminance(oi, Illumi_level/20);
        ...
        img{ii} = imresize(img{ii},[512,512]);
        max_value = max(img{ii}(:));  % 计算最大强度值
        threshold = 0.8 * max_value;  % 计算80%阈值
        img{ii}(img{ii} < threshold) = 0;  % 低于阈值的像素设为0
        imwrite(img{ii}, imgFilename); 
